...
Compaction involves summarizing or truncating earlier parts of the conversation to free up space while preserving essential information. This is the most straightforward approach to managing context in long-running sessions. The key challenge is deciding what to keep and what to summarize. Effective compaction requires the agent to identify which historical context is most relevant to the current task, which itself requires good judgment about the task at hand.

**Structured note-taking**

Rather than relying solely on the conversation history, agents can write structured notes or files that serve as external memory. This approach is analogous to how humans use notebooks or todo lists to track progress on complex projects. The agent can refer back to these notes in future turns, maintaining coherence without keeping everything in the context window.

**Multi-agent architectures**

For particularly complex tasks, a multi-agent architecture can distribute the context burden across multiple agents, each with a specialized role. For example, one agent might focus on planning and coordination, while others execute specific subtasks. This approach allows each agent to maintain a focused context window relevant to its specific responsibilities.

## Conclusion

Context engineering represents a fundamental shift in how we build AI applications. As models become more capable, the bottleneck moves from model intelligence to the quality and organization of context. Teams that master context engineering will be able to build more capable, reliable, and efficient agents.

The principles we've outlined—keeping context informative yet tight, using just-in-time retrieval, and employing specialized techniques for long-horizon tasks—provide a foundation for building effective agents. As the field continues to evolve, we expect context engineering to become an increasingly important discipline in AI development.

For more information on building effective agents, see our [Building effective agents](https://www.anthropic.com/research/building-effective-agents) post, and for practical guidance on prompt engineering, see our [documentation](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview).